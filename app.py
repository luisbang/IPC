...
# Variável para verificar se a autenticação foi bem-sucedida
autenticado = False

# Autenticação por senha
senha_usuario = st.text_input("Digite a senha:", type="password")

# Verificar a senha apenas se o botão de autenticação não foi clicado
if not autenticado:
    if st.button("Entrar") and senha_usuario == senha_correta:
        autenticado = True
...
📣Por favor identifica dízimos com o final 👉,01 nos depósitos 📣
    ...
    sugestao="www.googleforms.com"
    st.markdown(f"[Clique para enviar as sugestões]({sugestao})")
